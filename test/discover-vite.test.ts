...
import type { ExternalNode, RouteNode } from '../src/types.js';
...
describe('Vite + React discovery', () => {
  ...
  it('discovers routes from createBrowserRouter config', async () => {
    ...
    const paths = pages.map((n) => (n as RouteNode).path);
  ...
  it('marks all routes as Client rendering', async () => {
    ...
    for (const p of pages) {
      expect((p as RouteNode).rendering).toBe('Client');
    }
  });
  ...
  it('detects externals from VITE_ prefixed env vars', async () => {
    ...
    const names = externals.map((n) => (n as ExternalNode).name);
