...
export async function writeProbeCache(projectDir: string, cache: ProbeCache): Promise<void> {
  ...
  await writeFile(join(shipmapDir(projectDir), CACHE_FILE), JSON.stringify(cache, null, 2));
}

export async function writeLastReport(projectDir: string, report: unknown): Promise<void> {
  await ensureDir(projectDir);
  await writeFile(join(shipmapDir(projectDir), REPORT_FILE), JSON.stringify(report, null, 2));
}
...
export async function archiveReport(projectDir: string, report: unknown): Promise<void> {
  ...
  const timestamp = new Date()
    ...
    .replace(/:/g, '-')
    .replace(/\.\d+Z$/, 'Z');
  await writeFile(join(historyDir, `${timestamp}.json`), JSON.stringify(report, null, 2));
